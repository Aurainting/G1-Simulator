...
public:
  ElasticBand() = default;
  void Advance(const std::vector<double>& x, const std::vector<double>& dx) {
    std::vector delta_x = {0.0, 0.0, 0.0};
...
// constants
constexpr double syncMisalign =
    0.1; // maximum mis-alignment before re-sync (simulation seconds)
constexpr double simRefreshFraction =
    0.7; // fraction of refresh available for simulation
constexpr int kErrorLength = 1024; // load error string length
...
std::string getExecutableDir() {
  ...
  const std::string realpath = [&]() -> std::string {
    ...
    while (!success) {
      ...
      if (const std::size_t written =
              readlink(path, realpath_ptr.get(), buf_size);
          written < buf_size) {
        realpath_ptr.get()[written] = '\0';
        success = true;
      } else if (written == -1) {
        if (errno == EINVAL) {
  ...
  for (std::size_t i = realpath.size() - 1; i > 0; --i) {
    if (constexpr char kPathSep = '/'; realpath.c_str()[i] == kPathSep) {
...
void scanPluginLibraries() {
  ...
  if (const int n_plugin = mjp_pluginCount()) {
  ...
  if (const std::string executable_dir = getExecutableDir();
  ...
  mj_loadAllPluginLibraries(
      plugin_dir.c_str(),
      +[](const char* filename, const int first, const int count) {
...
mjModel* LoadModel(const char* file, mj::Simulate& sim) {
  ...
  mjModel* m_new = nullptr;
  if (mju::strlen_arr(filename) > 4 &&
      !std::strncmp(filename + mju::strlen_arr(filename) - 4, ".mjb",
                    mju::sizeof_arr(filename) - mju::strlen_arr(filename) +
                        4)) {
    m_new = mj_loadModel(filename, nullptr);
    if (!m_new) {
      mju::strcpy_arr(loadError, "could not load binary model");
    }
  } else {
    m_new = mj_loadXML(filename, nullptr, loadError, kErrorLength);
    // remove trailing newline character from loadError
    if (loadError[0]) {
      if (const auto error_length =
              static_cast<int>(mju::strlen_arr(loadError));
          loadError[error_length - 1] == '\n') {
...
void PhysicsLoop(mj::Simulate& sim) {
  ...
  while (!sim.exitrequest.load()) {
    if (sim.droploadrequest.load()) {
      ...
      mjModel* m_new = LoadModel(sim.dropfilename, sim);
      sim.droploadrequest.store(false);

      mjData* d_new = nullptr;
      if (m_new)
        d_new = mj_makeData(m_new);
      if (d_new) {
        sim.Load(m_new, d_new, sim.dropfilename);
        ...
        // allocate ctrl noise
        free(ctrl_noise);
        ctrl_noise = static_cast<mjtNum*>(malloc(sizeof(mjtNum) * m->nu));
        mju_zero(ctrl_noise, m->nu);
      } else {
    ...
    if (sim.uiloadrequest.load()) {
      ...
      mjModel* m_new = LoadModel(sim.filename, sim);
      mjData* d_new = nullptr;
      if (m_new)
        d_new = mj_makeData(m_new);
      if (d_new) {
        sim.Load(m_new, d_new, sim.filename);
      ...
      if (m) {
        ...
        if (sim.run) {
          ...
          if (sim.ctrl_noise_std != 0.0) {
            ...
            const mjtNum rate = mju_exp(-m->opt.timestep /
            ...
            for (int i = 0; i < m->nu; i++) {
              // update noise
              ctrl_noise[i] =
                  rate * ctrl_noise[i] + scale * mju_standardNormal(nullptr);
          ...
          const double slowdown =
              100 / mujoco::Simulate::percentRealTime[sim.real_time_index];
          ...
          // in-sync: step until ahead of cpu
            ...
            while (Seconds((d->time - syncSim) * slowdown) <
              ...
              if (!measured && elapsedSim != 0.0) {
                sim.measured_slowdown = static_cast<float>(
              ...
              if (param::config.enable_elastic_band == 1) {
                if (elastic_band.enable_) {
                  const std::vector x = {d->qpos[0], d->qpos[1], d->qpos[2]};
                  std::vector dx = {d->qvel[0], d->qvel[1], d->qvel[2]};
...
[[noreturn]] void* UnitreeSdk2BridgeThread() {
  ...
  ChannelFactory::Instance()->Init(param::config.domain_id,
...
void user_key_cb(GLFWwindow* window, const int key, int scancode, const int act,
                 int mods) {
  if (act == GLFW_PRESS) {
    if (param::config.enable_elastic_band == 1) {
      if (key == GLFW_KEY_9) {
...
int main(const int argc, char** argv) {
  ...
  param::config.load_from_yaml(proj_dir / "simulate" / "config.yaml");
  param::helper(argc, argv);
  if (param::config.robot_scene.is_relative()) {
    param::config.robot_scene =
        proj_dir / "assets" / param::config.robot / param::config.robot_scene;
  }
  ...
  std::thread unitree_thread(UnitreeSdk2BridgeThread);

  // start physics thread
  std::thread physics_thread_handle(&PhysicsThread, sim.get(),
  ...
  glfwSetKeyCallback(
      dynamic_cast<mj::GlfwAdapter*>(sim->platform_ui.get())->window_,
      user_key_cb);
  sim->RenderLoop();
